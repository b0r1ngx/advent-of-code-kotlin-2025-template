...
fun sumInvalidIdsPartTwo(ranges: List<String>): Long {
    val invalidIds = mutableListOf<Long>()

    ranges.forEach { range ->
        val (from, to) = range.split('-').map { it.toLong() }
        (outerLoop@ for (i in from..to) {
            val strValue = "$i"
            val patternMaxLength = strValue.length / 2
            for (j in 1..patternMaxLength) {
                val chunks = strValue.chunked(j)
...
fun main(args: Array<String>) {
    ...
    val sum = sumInvalidIdsPartTwo(input)
    println(sum)
}
